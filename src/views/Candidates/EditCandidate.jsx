...
import ExperienceAddForm from "../../components/Candidates/ExperienceAddForm";
...
class EditCandidate extends Component {
   constructor(props, context) {
      super(props, context);
      this.state = {
         candidateId: params.get("candidateId"),sess_id:cookies.get("c_csrftoken"),
         currentPageNumber: params.get("currPageNumber") ? params.get("currPageNumber") : 0,
      ...
      this.workAuthorizationList=[{"name":"B1"},{"name":"can work for any employer"},{"name":"canada Authorized"},{"name":"canadian citizen"},{"name":"citizen"},
      {"name":"current Employer only"}, {"name":"employment Auth.Document"},{"name":"employment Authorization Document"}, {"name":"GC"}, {"name":"GC-EAD"},
      {"name":"GreenCardHolder"},{"name":"H1B"}, {"name":"have H1 visa"},{"name":"India Authorized"}, {"name":"L1-A"}, {"name":"L1-B"}, {"name":"UScitizen"}]
   ...
   //authorization list
   onSelectAuthorizationList(selectedList, selectedItem) {
      let sts = []
      for (var i = 0; i < selectedList.length; i++) {
          sts[i] = (selectedList[i].name);
      }
      let sts_ = sts.join(", ");
      this.state.workAuthorizationList = sts_;
  }
  onRemoveAuthorizationList(selectedList, selectedItem) {
      let sts = []
      for (var i = 0; i < selectedList.length; i++) {
          sts[i] = (selectedList[i].name);
      }
   ...
   render() {
      ...
      const educationAddForm = this.state.showEducationAddFormCond ? <EducationAddForm candidateId={this.state.candidateId} sess_id={this.state.sess_id}  showLoadingBar={this.showLoadingBar} closeEducationAddForm={this.closeEducationAddForm}  /> : "";
      const experienceAddForm = this.state.showExperienceAddFormCond ? <ExperienceAddForm showLoadingBar={this.showLoadingBar} closeExperienceAddForm={this.closeExperienceAddForm} candidateId={this.state.candidateId} sess_id={this.state.sess_id}  /> : "";
      const skillsAddForm = this.state.showSkillsAddFormCond ? <SkillAddForm candidateId={this.state.candidateId} sess_id={this.state.sess_id} showLoadingBar={this.showLoadingBar} closeSkillAddForm={this.closeSkillAddForm}  /> : "";
      const thisObj = this;
     // console.log(this.state.candidateDetails);
      ...
      return (this.state.candidateDetails !== null && this.state.candidateDetails.firstName === "") ? (<div className="header bg-gradient-info pb-4 pt-5 pt-md-8">
         <Container fluid>
            <div className="header-body">
               <Row>
         ...
         <LoadingBar
            ...
            onLoaderFinished={0}
         />
         <div className="header bg-gradient-info pb-1 pt-2 pt-md-8">

            <Container fluid>
               <div className="header-body">
                  {(!this.state.candidateDetails.resumePath === "") ? (<div className="alert custom-alert custom-alert-danger icon-custom-alert alert-secondary-shadow fade show" role="alert">
                     <i className="la la-skull-crossbones alert-icon text-danger align-self-center font-30 mr-3"></i>
                     <div className="alert-text my-1">
                        <h5 className="mb-1 font-weight-bold mt-0">Warning</h5>
                        <span>File does not exists, will be updated soon.</span>
                     </div>
                  </div>) : ""}
                  {/* Card stats */}
                  <Row>
                     <Col lg="12" xl="12">
                        <h3 className="font-54 font-weight-bold mt-2 col-sm-8 float-left">Edit Candidate</h3>
                        <div className="float-right">
                           <button title="Preview Uploaded Resume" onClick={(e) => {
                              this.setState({ documentPreviewMain: (!this.state.documentPreviewMain) });
                              document.body.style.cursor = 'default';
                           }} className="btn btn btn-info btn-sm waves-effect waves-light">Preview Resume</button>

                           <a href={'../' + userType + '/candidates#' + this.state.currentPageNumber}
                              className="btn btn btn-dark ml-2 btn-sm waves-effect waves-light ">back</a>
                        </div>


                     </Col>
                     <Col lg="12" xl="12">
                        {/*<div className={classNames("custom-control", "custom-switch", "float-left", " ml-2")}>
                           <input type="checkbox" onChange={(e) => {
                              this.setState({ documentPreview: (!this.state.documentPreview) });
                              document.body.style.cursor = 'default';
                           }} className="custom-control-input" id="customSwitch1" defaultChecked={this.state.documentPreview} />
                           <Label className={classNames("custom-control-label")} for="customSwitch1">Generate document preview</Label>
                        </div>*/
                        }
         ...
         <div className="col-sm-12 pt-4">
            ...
            {/* <Container className="col-sm-6 float-left" fluid>
               <Row>
                  <div className="col">
                     <Card className="shadow">

                        {this.state.candidateDetails.firstName !== "" ?
                           <div className="">
                                 ...
                                 <div className="row">
                                    <div className="col-md-12 card-body">
                                       <div className="form-group">
                                          {(this.state.documentPreview) ? (<IFrame src={downloadUrl} loading={true} />) :
                                             <textarea className="bg-light form-control res_contents" defaultValue={this.state.candidateDetails.resumeDetails}></textarea>}
                                       </div>
                                    </div>
                                 </div></div></div> : ""}
                     ...
                     <div>
                              ...
                              <div className="col-md-2">
                                 <div className="form-group">
                                    ...
                                    <div className="input-group input-group-merge">
                                       ...
                                       <input placeholder="phone" type="number" className="form-control" name="number" defaultValue={this.state.candidateDetails.phone} onChange={(e) => { this.state.candidateDetails.phone = e.target.value }} />
                                    </div>
                              ...
                              <div className="col-md-2">
                                 <div className="form-group">
                                    ...
                                    <div className="input-group input-group-merge">
                                       ...
                                       <input placeholder="Alternate Phone" type="number" className="form-control" name="altphone" defaultValue={this.state.candidateDetails.altphone} onChange={(e) => { this.state.candidateDetails.altphone = e.target.value }} />
                                    </div>
                                    {/*<NumberFormat format="+1 (###) ###-####" mask="_" className="form-control" defaultValue={this.state.phone} name="phone" onChange={this.handleInput}/>*/}
                                 </div>
                              </div>
                              <div className="col-md-2">
                                 <div className="form-group w-100 candidate-workauth">
                                    ...
                                    <Multiselect className="gray-bg"
                                                        options={this.workAuthorizationList} // Options to display in the dropdown
                                                        //selectedValues={this.state.selectedStates} // Preselected value to persist in dropdown
                                                        onSelect={this.onSelectAuthorizationList} // Function will trigger on select event
                                                        onRemove={this.onRemoveAuthorizationList} // Function will trigger on remove event
                        ...
                        this.setState({ExperienceDetails:expeDetails}); */}
                           ...
                           <div className="row pl-3">
                              ...
                              <a href={'../' + userType + '/candidates'} className="btn btn-secondary waves-effect waves-light" style={{ marginRight: 10 + 'px' }}>
                              ...
                              <button type="button" onClick={(e) => this.handleUpdate(e, true)} className="btn ml-2 btn-info  waves-effect waves-light">
                                 {/*<FontAwesomeIcon icon={faSave}/>*/}<span className="btn-inner--icon"><i className="fas fa-save mr-2"></i></span>  Save and Continue Edit</button>
               ...
               <Card className={"mt-4"}>
                  <CardHeader>
                     <div className="row align-items-center">
                        <Col>
                           <h3 className="card-title"><i className="dripicons-checklist pr-2 fa-10"></i> TECHNICAL SKILLS</h3>
                        </Col>

                        <div className="col-auto">
                           {this.state.skillsList.length === 1 && this.state.skillsList[0].skills === "" ?
                              <button onClick={(e) => this.setState({ showSkillsAddFormCond: true })} className="btn btn-sm btn-info d-inline-block">Add</button> : ""}
                        </div>
                     </div>
                  </CardHeader>
                  <CardBody className="pt-1">
                     {this.state.skillsList.length !== 0 && this.state.skillsList[0].skills !== "" ? <ul className="list-group-flush list-group pt-0 mt-0 mb-0">{this.state.skillsList.map(function (obj, index) {
                        return <li className="list-group-item pr-0 pl-0  pb-2">
                           <div className="col-sm-10 float-left pt-1 pl-0 pr-0 ml-0 mr-0">
                              <div className="card-subtitle mt-1 float-left"><i className="float-left dripicons-toggles mr-2 fa-10 text-info pt-1"></i><span className="offer-1 text-muted">{obj.skills}</span> </div>
                           </div>
                           <span className="badge  badge-pill float-right button-items">
                              <button className="btn btn-sm btn-soft-success btn-circle" onClick={(e) => thisObj.updateSkill(e, obj.id)}><i className="dripicons-pencil" aria-hidden="true"></i></button>
                              <button type="button" className="btn btn-sm btn-soft-danger btn-circle" onClick={(e) => thisObj.deleteAttribute(e, "skills", obj.id)}><i className="dripicons-trash"></i></button>
                           </span></li>
               ...
               <Card>
                  <CardHeader>
                     <div className="row align-items-center">
                        ...
                        <div className="col-auto">
                           <button onClick={(e) => this.setState({ showEducationAddFormCond: true })} className="btn btn-sm btn-info d-inline-block">Add</button>
                        </div>
                     </div>
                  </CardHeader>
               ...
               <Card>
                  <CardHeader>
                     <div className="row align-items-center">
                        ...
                        <div className="col-auto">
                           <button className="btn btn-sm btn-info d-inline-block" onClick={(e) => this.setState({ showExperienceAddFormCond: true })}>Add</button>
                        </div>
                     </div>
                  </CardHeader>
                  <CardBody>
                                 ...
                                 <div className="row">
                                    ...
                                    <div className="col-md-2 float-left mt-4 pt-1">
                                       {(this.state.candidateDetails.resumePath !== "") ? <a download title="Download the current resume file" href={`${apiUrl}downloadResume?candidateId=${this.state.candidateId}&sess_id=${cookies.get("c_csrftoken")}`} className="btn  btn-warning  btn-square btn-outline-dashed waves-effect waves-light">
                                          <i className="fa fa-download" ></i> Download</a> : "File not found"}
                                    </div>
                                 </div>
                                 <button type="button" onClick={this.handleDelete} title="Delete old resume" className="btn btn btn-danger waves-effect waves-light btn-icon"><span className="btn-inner--icon"><i className="fa fa-trash"></i></span> Delete</button>
                              </div> </div>
