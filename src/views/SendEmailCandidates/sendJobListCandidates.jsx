...
class SendJobListCandidates extends Component {
  ...
  handleJobDescription = (e)=>{
    ...
    axios.get(apiUrl + "getJobDescriptionDetails",{ params:{ sess_id: this.state.sess_id, jobId:selectedId } })
    .then(({ data }) => {
        if(data.statusResponse)
          this.setState({jobTemplate:data.jobTemplate});
  ...
  componentDidMount() {
    ...
    axios.get(apiUrl+"getAllJobsSelectList",{params:{sess_id:this.state.sess_id}}).then(({data})=>{
      this.jobPostList = data.results;
    });
    
    axios.get(apiUrl + "getJobEmailCandidateList",{ params:{ sess_id: this.state.sess_id, pageId:pageId } })
    .then(({ data }) => {
        if(data.statusResponse)
          this.setState({profiles:data.candidatesList,fromEmail:data.fromEmail,sessionId:data.sessionId, jobTemplate:data.jobTemplate});
        //this.setState({ countriesList: data,statesList:[],state:"" })
    })
    .catch((err) => { })
        ...
        <div className="col-sm-12">
          <div className="page-title-box">
            <div className="row pl-2">
              <div className="col ">
                <h4 className="page-title ">Send Job</h4>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="row p-3">
                ...
                <div className="col-sm-12">
                  <div className="form-group" >
                    ...
                    <select className="form-control gray-bg" name="emailSubject" onChange={this.handleJobDescription}>
                      <option value="">select a job posting</option>
                      {this.jobPostList.map((post,index) =>{return <option key={index} value={post.id}>{post.jobCode} {post.jobTitle}</option>})}
                    </select>
                ...
                <div className="col-sm-12">
                  <div className="form-group" >
                    ...
                    {this.state.jobTemplate &&
                    <CKEditor
                    editor={ ClassicEditor }
                    data={this.state.jobTemplate}
                    onReady={ editor => {
                    ...
                    onFocus={ ( event, editor ) => {
                      ...
                      {/*
                          ...
                          initData={this.state.jobTemplate}
                          data={this.state.jobTemplate}
                          name="Mail content"
                          className="templateDescriptionEditor"
...
export default SendJobListCandidates;