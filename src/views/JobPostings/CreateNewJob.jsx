...
import { apiUrl,addNewJob, style } from "../../variables/Variables.jsx";
...
class CreateNewJob extends Component {
    constructor(props, context) {
        ...
        this.onSelectTaxTermsList = this.onSelectTaxTermsList.bind(this);
        this.onRemoveTaxTermsList = this.onRemoveTaxTermsList.bind(this)
    ...
    componentDidMount() {
        ...
        axios.get(apiUrl + "getAllConfigLists",{//3
            ...
            if(data.statusResponse===true)
                data.list.map((det)=>{
                    taxTermsTmp.push({"name":det.ConfigName})
                });
            //console.log(taxTermsTmp)
           this.setState({taxTerms:taxTermsTmp})
        });

        //getting department array for options
        axios.get(apiUrl + "getAllConfigLists",{//3
            params: {
                sess_id: this.state.sess_id,
            ...
            if(data.statusResponse===true)
                data= data.list.map((det)=>det.ConfigName);
    ...
    onSelectTaxTermsList(selectedList){
        selectedList=selectedList.map(taxTerms=>taxTerms.name).join(",");
        this.setState({orgInfoTaxTerms:selectedList})
    }

    onRemoveTaxTermsList(selectedList){
        selectedList=selectedList.map(taxTerms=>taxTerms.name).join(",");
        this.setState({orgInfoTaxTerms:selectedList})
    }
                                        ...
                                        <div className="col-sm-4 pl-0 ml-4 float-left">
                                            ...
                                            <div className="form-group mb-2 ">
                                                ...
                                                <select className="form-control gray-bg" name="orgInfoDepartment" onChange={this.handleInput}>
                                                    <option value="">select</option>
                                                    {this.state.departments && this.state.departments.map(department => <option value={department}>{department}</option>)}
                                        ...
                                        <div className="col-sm-4 pl-0  float-right">
                                            <div className="form-group mb-2 ">{/*this.state.taxTerms*/}
                                                ...
                                                <Multiselect className="form-control-sm"
                                                    ...
                                                    //selectedValues={this.state.selectedStates} // Preselected value to persist in dropdown
                                                    onSelect={this.onSelectTaxTermsList} // Function will trigger on select event
                                                    onRemove={this.onRemoveTaxTermsList} // Function will trigger on remove event
                                                    displayValue="name" // Property name to display in the dropdown options
                                                    name="orgInfTaxTerms"
                                                    showCheckbox="true"
                                                    placeholder="Select" 
                                                />
                                                {console.log(this.state.taxTerms)}
                                            </div>
